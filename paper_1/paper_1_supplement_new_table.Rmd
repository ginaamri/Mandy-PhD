...
quest.raw.data.column <- quest.raw.data %>% select(itemID, value)
view(quest.raw.data.column)
# changing long format into wide format 
quest.raw.data.wide <- spread (quest.raw.data.column, itemID, value)
view(quest.raw.data.wide)

quest.raw.data.wide <- as_tibble(quest.raw.data) %>%          
                          select(scale, value) %>%             # select columns of interest
                          mutate(id = row_number()) %>%         # create a row identifier
                          spread(scale, value)                 # reshape dataset
...
(1) Classroom management
```{r classroom management line plots,  fig.width=6, fig.height = 6, message = FALSE, warning = FALSE , echo=FALSE, fig.cap = Items for Classroom Management}
...
\newpage
(8) Natural behaviour
```{r Natural behaviour line plots,  fig.width=6, fig.height = 6, message = FALSE, warning = FALSE , echo=FALSE, fig.cap = Items for Natural beaviour}
