...
# prepare data (selected from questionnaire data)
et.raw.data<-read.table("./data/Aperol_pilot_02_glasses_fixation_metrics.tsv", dec=",", sep="\t", header=T) 
...
# create a basic table (tibble) using tidyverse functions
et.quest.table <- et.raw.data %>%
  summarise("M Fixation Number" = round(mean(Number_of_whole_fixations), 2),
            "SE Fixation Number" = round(se(Number_of_whole_fixations), 2),
            "M Duration Fixation" = round(mean(Total_duration_of_whole_fixations)),
            "SE Duration Fixation" = round(se(Total_duration_of_whole_fixations))
            )

# format and insert table in manuscript
apa_table(
  et.quest.table,
  caption = "Number and Duration of fixations",
  # note = "Write Note here", 
  escape = TRUE, # if TRUE special Latex characters are escaped; if this is turned to F captions cannot be rendered. I don't know why...
  placement = "h", # position of table in page:  exact location (h), at the top (t), bottom (b)
  font_size = "small" # options are tiny, scriptsize, footnotesize, small, normalsize (default), large, Large, LARGE, huge, Huge
...
The recorded lessons were coded in a post-hoc procedure with the coding software MAXQDA by previously trained raters (@kuckartz2019analyzing). The following coding scheme has been developed for coding:
