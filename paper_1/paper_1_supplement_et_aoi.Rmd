...
et.raw.data <- et.raw.data %>% mutate(total_fixations = sum(Total_duration_of_fixations))


# create new columns that give proportion of the duration of looking at AOIs
# relative duration divided by total duration is the proportion
proportion.et.data <- et.raw.data %>% group_by(Group, AOI) %>% 
...
et.raw.data <- et.raw.data %>% mutate(total_visits = sum(Total_duration_of_Visit))


# create new columns that give proportion of the duration of looking at AOIs
# relative duration divided by total duration is the proportion
proportion.et.data <- et.raw.data %>% group_by(Group, AOI) %>% 
                                      mutate(proportion_visits = Total_duration_of_Visit/Total)



### Plot data TOTAL FIXATIONS
total.fix.plot <- ggplot(et.raw.data, aes(x = AOI, y = Total_duration_of_fixations, fill = AOI)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
  scale_x_discrete(guide = guide_axis(angle = 70)) +
  geom_boxplot()+
  geom_jitter(color="black", size=0.4, alpha=0.5) +
...
### Plot Number of Fixations
number.plot <- ggplot(et.raw.data, aes(x = AOI, y = Number_of_fixations, fill = AOI)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
  scale_x_discrete(guide = guide_axis(angle = 70)) +
  geom_boxplot()+
  geom_jitter(color="black", size=0.4, alpha=0.5) +
...
total.visit.plot <- ggplot(et.raw.data, aes(x = AOI, y = Total_duration_of_Visit, fill = AOI)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
  scale_x_discrete(guide = guide_axis(angle = 70)) +
  geom_boxplot()+
  geom_jitter(color="black", size=0.4, alpha=0.5) +
